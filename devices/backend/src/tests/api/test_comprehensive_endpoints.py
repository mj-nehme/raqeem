...
class TestScreenshotEndpoints:
    ...
    async def test_upload_screenshot_success(self, init_test_db):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            data = {"device_id": sample_uuid, "resolution": "1920x1080"}
...
class TestUserEndpoints:
    ...
    async def test_create_user_invalid_email(self, init_test_db):
        ...
        invalid_user = {"username": "testuser", "email": "invalid-email", "password": "password"}
    ...
    async def test_malformed_json_request(self, init_test_db):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post(
                "/api/v1/devices/register", content="invalid json", headers={"Content-Type": "application/json"}
            )
    ...
    async def test_cpu_usage_boundaries(self, init_test_db):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            for case in test_cases:
                response = await client.post(
                    f"/api/v1/devices/{sample_uuid}/metrics", json={"cpu_usage": case["cpu_usage"]}
                )
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            for case in test_cases:
                response = await client.post(
                    ...
                    json={"memory_used": case["memory_used"], "memory_total": case["memory_total"]},
                )
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            for case in test_cases:
                response = await client.post(
                    ...
                    json={"latitude": case["latitude"], "longitude": case["longitude"]},
                )
                # Endpoint doesn't exist, should be 404
                assert response.status_code in [200, 404, 422]
    ...
    async def test_large_payload_handling(self, init_test_db):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post(
                ...
                json=[{"activity_type": "app_launch", "description": large_description}],
            )
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            for i in range(5):
                response = await client.post(
                    ...
                    json={"deviceid": str(uuid.uuid4()), "device_name": f"Device {i}", "device_type": "laptop"},
                )
